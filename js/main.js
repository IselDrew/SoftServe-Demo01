...
document.querySelector("#f2 button").addEventListener("click", e => {
  e.preventDefault();
  const obj = {};
  obj["verticles"] = document
    .querySelector('#f2 input[name="verticles"]')
    .value.toUpperCase();
  obj[`${obj.verticles[0].toLowerCase()}`] = Math.abs(
    Number(document.querySelector('#f2 input[name="tF"]').value)
  );
  obj[`${obj.verticles[1].toLowerCase()}`] = Math.abs(
    Number(document.querySelector('#f2 input[name="tS"]').value)
  );
  obj[`${obj.verticles[2].toLowerCase()}`] = Math.abs(
    Number(document.querySelector('#f2 input[name="tT"]').value)
  );
  dataTask3.push(obj);
...
document.querySelector("#f3 button").addEventListener("click", e => {
  ...
  const number = Math.abs(
    Number(document.querySelector('#f3 input[name="number"]').value)
  );
  ...
  if (result.reason) {
    ...
    document.getElementById(
      "3"
    ).innerHTML = `Наибольший палиндром -> ${result}`;
  }
});
